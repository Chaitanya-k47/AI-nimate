...
# --- Configuration (Blueprint will fill these in) ---
...
try:
    ...
    # 6. Set up export options AND EXPLICITLY DEFINE THE BAKE RANGE
    export_options = unreal.AnimSeqExportOption()
    export_options.export_transforms = True

    unreal.log(f"Received bake range from Blueprint: Frame {START_FRAME} to {END_FRAME}")

    # Tell the baker to use this custom range instead of its faulty default.
    export_options.use_custom_time_range = True
    export_options.custom_start_frame = unreal.FrameNumber(START_FRAME)
    export_options.custom_end_frame = unreal.FrameNumber(END_FRAME)
